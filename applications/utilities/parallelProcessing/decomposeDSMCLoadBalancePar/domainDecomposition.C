...
//                    << "    size     :" << size << endl;
                ...
                if (subPatchID[i] == -1)
                {
                    ...
                    procPatches[nPatches] =
                        ...
                        (
                          ...
                          + "to"
                          + Foam::name(curNeighbourProcessors[procPatchI]),*/ // REMOVED VINCENT (see /home/vincent/OpenFOAM/OpenFOAM-v1606+/src/OpenFOAM/lnInclude/processorPolyPatch.H)
                            size,
                            curStart,
                            nPatches,
                            procMesh.boundaryMesh(),
        ...
        const IOstreamOption writeOpt
        (
            processorDb.writeFormat(),
            processorDb.writeVersion(),
            processorDb.writeCompression()
        );

        // In the current v2506 objectRegistry path, procMesh.write() does not
        // reliably materialize the primitive mesh files for this transient
        // processor database. Force them out explicitly so the later
        // field-transfer step can reopen the decomposed mesh.
        pointIOField
        (
            IOobject
            (
        ...
        (
            ...
            procMesh.faceNeighbour()
        ).writeObject(writeOpt, true);

        procMesh.boundaryMesh().writeObject(writeOpt, true);
