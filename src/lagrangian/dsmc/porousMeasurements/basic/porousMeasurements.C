...
)
{
    ...
    porousMeasurementModel[0] = std::toupper(porousMeasurementModel[0]);
    porousMeasurementModel =
        "dsmc" + porousMeasurementModel + "PorousMediumMeasurements";

    Info<< "Selecting the porous measurement model:" << tab
        << porousMeasurementModel << "\n" << endl;

    auto* ctorPtr = fvMeshConstructorTable(porousMeasurementModel);
    ...
    return autoPtr<porousMeasurements>(ctorPtr(t, mesh, cloud));
}
