    ...
    relax_(true)
{
    ...
    const List<word> reactants(Foam::hyCompat::lookup(dict, "reactants"));
    reactantIds_.setSize(reactants.size(), -1);
    reactantTypes_.setSize(reactantIds_.size(), -1);

    forAll(reactantIds_, r)
    {
        reactantIds_[r] = Foam::hyCompat::indexOf(cloud_.typeIdList(), reactants[r]);
...
{
    word dsmcReactionName(Foam::hyCompat::lookup(dict, "reactionModel"));

    Info<< "Selecting the reaction model "
         << dsmcReactionName << endl;

    auto* ctorPtr = dictionaryConstructorTable(dsmcReactionName);
    ...
    return autoPtr<dsmcReaction>(ctorPtr(t, cloud, dict));
}
