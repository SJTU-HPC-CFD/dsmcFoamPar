...
void exchangeQK::setProperties()
{
    ...
    const wordList productsExchange(propsDict_.get<wordList>("exchangeProducts"));
...
)
{
    ...
    if (chiB <= VSMALL || translationalEnergy <= VSMALL)
    {
        return;
    }
    ...
    const scalar exponent = 1.5 - omegaPQ;
    const scalar aDash =
        aCoeff_
       *(
            pow(max(chiB, VSMALL), bCoeff_)
           *exp(lgamma(max(chiB, VSMALL)))
           /exp(lgamma(max(chiB + bCoeff_, VSMALL)))
        );

    scalar activationEnergy =
        (
            aDash*pow(max(TColl/273.0, VSMALL), bCoeff_)
    ...
    do
    {
        ...
        if (collisionEnergy > activationEnergy && collisionEnergy > VSMALL)
        {
            if (activationEnergy > kBByThetaVP && kBByThetaVP > VSMALL)
            {
                summation = 0.0;
                ...
                for (label i = 0; i <= iaP; i++)
                {
                    ...
                    summation += pow(vibBase, exponent);
                }
            }

            const scalar reactionBase = max(1.0 - activationEnergy/collisionEnergy, 0.0);
            reactionProbability = pow(reactionBase, exponent)/max(summation, VSMALL);
...
:
    ...
    heatOfReactionExchangeJoules_
    (
        propsDict_.get<scalar>("heatOfReactionExchange")
    ...
    if (writeRatesToTerminal_)
    {
        ...
        forAll(cellOccupancy, c)
        {
            ...
            forAll(parcelsInCell, pIC)
            {
                ...
                const label pos = reactantIds_.find(p->typeId());
