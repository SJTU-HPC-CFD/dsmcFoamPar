...
void chargeExchangeQK::setProperties()
{
    ...
    const wordList productsChargeExchange(Foam::hyCompat::lookup(propsDict_, "chargeExchangeProducts"));
...
:
    ...
    (
        Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "heatOfReactionChargeExchange"))
       *physicoChemical::k.value()
    ),
    aCoeffChEx_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "aCoeff"))),
    bCoeffChEx_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "bCoeff"))),
    volume_(0.0)
{}
...
)
{
    ...
    const label reactantPId = Foam::hyCompat::indexOf(reactantIds_, typeIdP);
    const label reactantQId = Foam::hyCompat::indexOf(reactantIds_, typeIdQ);
    ...
    if (writeRatesToTerminal_)
    {
        ...
        forAll(cellOccupancy, c)
        {
            ...
            forAll(parcelsInCell, pIC)
            {
                ...
                const label pos = Foam::hyCompat::indexOf(reactantIds_, p->typeId());
