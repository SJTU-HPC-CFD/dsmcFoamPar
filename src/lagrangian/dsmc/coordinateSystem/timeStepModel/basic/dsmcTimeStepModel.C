...
)
{
    ...
    timeStepModel[0] = std::toupper(timeStepModel[0]);
    timeStepModel = "dsmc" + timeStepModel + "TimeStepModel";

    Info<< "Selecting the time-step model:" << tab << timeStepModel
        << "\n" << endl;

    auto* ctorPtr = fvMeshConstructorTable(timeStepModel);
    ...
    return autoPtr<dsmcTimeStepModel>(ctorPtr(t, mesh, cloud));
}
