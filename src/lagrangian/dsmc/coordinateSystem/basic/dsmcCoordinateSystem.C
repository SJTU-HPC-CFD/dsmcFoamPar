...
:
    ...
    nParticlesOrg_
    (
        Foam::hyCompat::toScalar(cloud.particleProperties(), "nEquivalentParticles")
    ),
...
)
{
   const word& coordSystem =
    ...
    auto* ctorPtr = fvMeshConstructorTable(coordSystem);
    ...
    return autoPtr<dsmcCoordinateSystem>(ctorPtr(t, mesh, cloud));
}
