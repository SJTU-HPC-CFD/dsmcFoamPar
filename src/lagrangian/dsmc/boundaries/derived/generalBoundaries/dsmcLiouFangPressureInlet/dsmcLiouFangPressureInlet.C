...
addToRunTimeSelectionTable(dsmcGeneralBoundary, dsmcLiouFangPressureInlet, dictionary);
    ...
    n_()
{
    ...
    n_ = inletPressure_ / (physicoChemical::k.value()*inletTemperature_); // equation 32 (I have defined pressure and temperature at inlet, Liou an Fang use pressure and mass density).
...
void dsmcLiouFangPressureInlet::updateProperties(const dictionary& newDict)
{
...
{
    inletPressure_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "inletPressure"));

    inletTemperature_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "inletTemperature"));

    theta_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "theta"));
    ...
    forAll(moleculesReduced, iD)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcLiouFangPressureInlet::setProperties()")
    ...
    {
        moleFractions_[iD] = Foam::hyCompat::toScalar
        (
            moleFractionsDict, moleculesReduced[iD]
        );
    }
