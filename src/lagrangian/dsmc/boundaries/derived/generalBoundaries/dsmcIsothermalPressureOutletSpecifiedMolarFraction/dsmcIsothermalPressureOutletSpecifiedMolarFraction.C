...
#include "dsmcIsothermalPressureOutletSpecifiedMolarFraction.H"
...
defineTypeNameAndDebug(dsmcIsothermalPressureOutletSpecifiedMolarFraction, 0);

addToRunTimeSelectionTable(dsmcGeneralBoundary, dsmcIsothermalPressureOutletSpecifiedMolarFraction, dictionary);
...
dsmcIsothermalPressureOutletSpecifiedMolarFraction::dsmcIsothermalPressureOutletSpecifiedMolarFraction
(
    Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud,
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::initialConfiguration()
{}

void dsmcIsothermalPressureOutletSpecifiedMolarFraction::calculateProperties()
{
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::controlParcelsBeforeMove()
{
    ...
    forAll(accumulatedParcelsToInsert_, iD)   // I Added.
    {
        ...
        forAll(accumulatedParcelsToInsert_[iD], f)
        {
            ...
            scalar faceTemperature = outletTemperature_;
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::controlParcelsBeforeCollisions()
{

}

void dsmcIsothermalPressureOutletSpecifiedMolarFraction::controlParcelsAfterCollisions()
{
    nTimeSteps_ += 1.0;
...
//     {
//         Pout << "dsmcIsothermalPressureOutletSpecifiedMolarFraction outlet velocity correction = " << velocityCorrection[(faces_.size()/2)] << endl;
//     }
//
//     if(faces_.size() > VSMALL)
//     {
//         Pout << "dsmcIsothermalPressureOutletSpecifiedMolarFraction outlet velocity = " << outletVelocity_[(faces_.size()/2)] << endl;
//     }
    ...
    forAll(accumulatedParcelsToInsert_, iD)
    {
        ...
        forAll(accumulatedParcelsToInsert_[iD], f)
        {
            ...
            accumulatedParcelsToInsert_[iD][f] +=
                moleFractions_[iD]*
                (
                    fA*outletNumberDensity_*deltaT*mostProbableSpeed
                    *
                    (
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::output
(
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::updateProperties(const dictionary& newDict)
{
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::setProperties()
{
    outletPressure_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "outletPressure"));

    outletTemperature_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "outletTemperature"));

    outletNumberDensity_ = outletPressure_/(1.3806e-23*outletTemperature_);

    const List<word> molecules(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "typeIds")));

    if(molecules.size() == 0)
    {
        FatalErrorIn("dsmcIsothermalPressureOutletSpecifiedMolarFraction::setProperties()")
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcIsothermalPressureOutletSpecifiedMolarFraction::dsmcIsothermalPressureOutletSpecifiedMolarFraction()")
    ...
    {
        moleFractions_[i] = Foam::hyCompat::toScalar
        (
            moleFractionsDict, moleculesReduced[i]
        );
    }
...
void dsmcIsothermalPressureOutletSpecifiedMolarFraction::setNewBoundaryFields()
{
