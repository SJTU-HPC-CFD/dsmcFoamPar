...
void dsmcCatalyticWallPatch::setProperties()
{
    ...
    const List<word> inputMolecules
                      (Foam::hyCompat::lookup(propsDict_, "moleculesToBeCatalysed"));
    ...
    forAll(inputMolecules, i)
    {
        ...
        if(Foam::hyCompat::indexOf(inputMoleculesReduced, moleculeName) == -1)
        {
    ...
    const List<word> outputMolecules(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "catalysedMolecules")));
    ...
    forAll(outputMolecules, i)
    {
        ...
        if(Foam::hyCompat::indexOf(outputMoleculesReduced, moleculeName) == -1)
        {
    ...
    if(heatOfReaction_.size() == inputMolecules.size())
    {
        ...
        {
            heatOfReaction_[i] = Foam::hyCompat::toScalar
            (
                propsDict_.subDict("heatOfReaction"), inputMoleculesReduced[i]
            );
        }
    }
    else
