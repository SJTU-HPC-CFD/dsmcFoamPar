...
//     time_(t, timeDict_),
    patchName_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(boundaryDict_, "patchName"))),
    patchId_(0),
    faces_(),
    nFaces_(0),
...
{
    word dsmcGeneralBoundaryName(Foam::hyCompat::lookup(dict, "boundaryModel"));

    Info<< "Selecting dsmcGeneralBoundaryModel "
         << dsmcGeneralBoundaryName << endl;

    auto* ctorPtr = dictionaryConstructorTable(dsmcGeneralBoundaryName);
