    ...
    writeInCase_(true)
{
    ...
    const word patchName = boundaryDict_.get<word>("patchName");
...
{
    word dsmcCyclicBoundaryName(Foam::hyCompat::lookup(dict, "boundaryModel"));

    Info<< "Selecting dsmcCyclicBoundaryModel "
         << dsmcCyclicBoundaryName << endl;

    auto* ctorPtr = dictionaryConstructorTable(dsmcCyclicBoundaryName);
