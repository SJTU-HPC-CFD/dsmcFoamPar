...
)
{
    ...
    auto* ctorPtr = dictionaryConstructorTable(dsmcFieldName);
...
)
{
    ...
    {
        time_.resetFieldsAtOutput() = timeDict_.get<Switch>("resetAtOutput");
    }
    ...
    scalar resetAtOutputUntilTime = 0.0;

    if
    (
           time_.resetFieldsAtOutput()
        && timeDict_.readIfPresent("resetAtOutputUntilTime", resetAtOutputUntilTime)
    )
    {
