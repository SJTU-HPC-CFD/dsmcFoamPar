...
//     regionId_( -1),
//     Foam::hyCompat::lookup(faceZoneName_(Foam::hyCompat::lookup(propsDict_, "surfaceZoneName"))),
    regionIds_(),
    faceZoneNames_(),
    ...
    massFluxZone_()
{
   ...
   // choose molecule ids to sample
    ...
    const List<word> zoneNames(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "faceZoneNames")));
    ...
    forAll(zoneNames, i)
    {
        ...
        if(Foam::hyCompat::indexOf(regionNames, zoneName) == -1)
        {
...
//         const labelList& faces = faceZones[regionId_];
        ...
        forAll(regionIds_, r)
        {
            ...
            forAll(faces, f)
            {
                ...
                forAll(molIdFlux, id)
                {
                    if(Foam::hyCompat::indexOf(molIds_, id) != -1)
                    {
