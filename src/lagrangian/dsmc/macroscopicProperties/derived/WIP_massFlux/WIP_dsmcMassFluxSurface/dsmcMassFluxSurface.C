...
:
    ...
    regionId_( -1),
    faceZoneName_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "faceZoneName"))),
    zoneSurfaceArea_(0.0),
    typeIds_(),
    fluxDirection_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "fluxDirection"))),
    molsZone_(0.0),
    massZone_(0.0),
...
void dsmcMassFluxSurface::calculateField()
{
    ...
    forAll(faces, f)
    {
        ...
        forAll(molIdFlux, id)
        {
            if(Foam::hyCompat::indexOf(typeIds_, id) != -1)
            {
