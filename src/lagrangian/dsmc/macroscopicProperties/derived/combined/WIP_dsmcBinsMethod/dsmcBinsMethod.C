...
:
    ...
    regionName_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "zoneName"))),
    regionId_(-1),
    ...
    wallRadius_(0.0)
{
    ...
    {
        averagingAcrossManyRuns_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "averagingAcrossManyRuns")));
    ...
    {
        permeabilityMeasurements_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "permeabilityMeasurements")));
    ...
    {
        rotationalMeasurements_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "rotationalMeasurements")));

        // read in stored data from dictionary
        if(rotationalMeasurements_)
        {
            wallVelocity_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "wallVelocity"));

            wallRadius_ = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "wallRadius"));
    ...
    if (propsDict_.found("outputProperties"))
    {
        ...
        const List<word> measurements(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "outputProperties")));
        ...
        forAll(measurements, i)
        {
            ...
            if(Foam::hyCompat::indexOf(propertyNames, propertyName) == -1)
            {
        ...
        if(Foam::hyCompat::indexOf(propertyNames, "density") == -1)
        {
            outputField_[0] = false;
        }

        if(Foam::hyCompat::indexOf(propertyNames, "velocity") == -1)
        {
            outputField_[1] = false;
        }

        if(Foam::hyCompat::indexOf(propertyNames, "temperature") == -1)
        {
            outputField_[2] = false;
        }

        if(Foam::hyCompat::indexOf(propertyNames, "pressure") == -1)
        {
...
void dsmcBinsMethod::calculateField()
{
    ...
    forAll(cells, c)
    {
        ...
        forAll(molsInCell, mIC)
        {
            ...
            label iD = Foam::hyCompat::indexOf(typeIds_, p->typeId());
