...
void dsmcZoneBounded::setBoundBoxes()
{
    ...
    PtrList<entry> boxList(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "boxes")));
...
//     instantaneous_ = false;
    ...
    {
        instantaneous_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "instantaneous")));
    }

    // instantaneous
    const scalar& deltaT = time_.mdTimeInterval().deltaT();
    scalar writeInterval = Foam::hyCompat::toScalar(t.controlDict(), "writeInterval");
