...
:
    ...
    time_(t, timeDict_),
    timePeriod_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(timeDict_, "initialTimePeriod"))), //temp
    initialTime_(time_.time().startTime().value()),
    regionName_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(controllerDict_, "zoneName"))),
    regionId_(-1),
    control_(true),
    ...
    writeInCase_(true)
{
    ...
    control_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(controllerDict_, "controlSwitch")));
...
{
    word dsmcStateControllerName(Foam::hyCompat::lookup(dict, "stateControllerModel"));

    Info<< "Selecting dsmcStateController "
         << dsmcStateControllerName << endl;

    auto* ctorPtr = dictionaryConstructorTable(dsmcStateControllerName);
