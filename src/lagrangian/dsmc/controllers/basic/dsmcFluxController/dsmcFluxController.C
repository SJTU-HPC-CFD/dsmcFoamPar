...
{
    word dsmcFluxControllerName(Foam::hyCompat::lookup(dict, "fluxControllerModel"));

    Info<< "Selecting fluxController "
         << dsmcFluxControllerName << endl;

    auto* ctorPtr = dictionaryConstructorTable(dsmcFluxControllerName);
...
void dsmcFluxController::setFacesInfo()
{
    ...
    if(Pstream::parRun())
    {
        ...
        forAll(mesh_.boundaryMesh(), patchI)
        {
            ...
            if (isA<processorPolyPatch>(patch))
            {
                for (label p = 0; p < patch.size(); p++)
                {
                    ...
                    label faceId = Foam::hyCompat::indexOf (faces, patchFaceI);
        ...
        forAll(faces, f)
        {
            ...
            if(Foam::hyCompat::indexOf(processorFaces, faceI) == -1)
            {
...
{
    const label f = Foam::hyCompat::indexOf(controlZone(), faceI);
