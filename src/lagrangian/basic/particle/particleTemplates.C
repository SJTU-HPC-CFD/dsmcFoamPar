...
void Foam::particle::readFields(CloudType& c)
{
    ...
    //if (procIO.headerOk())
    if (procIO.typeHeaderOk<IOList<label>>(false)) // NEW VINCENT
    {
...
template<class TrackData>
Foam::label Foam::particle::track(const vector& endPosition, TrackData& td, bool DSMC/*, indexedOctree<treeDataFace> faceTree*/)
{
    faceI_ = -1;
    ...
    while (!onBoundary() && stepFraction_ < 1.0 - SMALL)
    {
        stepFraction_ += trackToFace(endPosition, td, DSMC/*, faceTree*/)*(1.0 - stepFraction_);
    }
