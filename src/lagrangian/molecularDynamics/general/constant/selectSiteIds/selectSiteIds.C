...
:
    siteIds_()
{
    const List<word> sites(Foam::hyCompat::lookup(dict, "siteIds"));
    ...
    if(sites.size() > 0)
    {
        ...
        else
        {
            ...
            forAll(sites, i)
            {
                ...
                if(Foam::hyCompat::indexOf(sitesReduced, siteName) == -1)
                {
            ...
            forAll(sitesReduced, i)
            {
                ...
                label siteId(Foam::hyCompat::indexOf(cP.siteIds(), siteName));
...
:
    siteIds_()
{
    const List<word> sites(Foam::hyCompat::lookup(dict, siteIdsHeader));
