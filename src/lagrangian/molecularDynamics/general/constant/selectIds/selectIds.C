...
:
    molIds_()
{
    const List<word> molecules(Foam::hyCompat::lookup(dict, "molIds"));
    ...
    if(molecules.size() > 0)
    {
        ...
        else
        {
            ...
            forAll(moleculesReduced, i)
            {
                ...
                label molId(Foam::hyCompat::indexOf(cP.molIds(), moleculeName));
...
:
    molIds_()
{
    const List<word> molecules(Foam::hyCompat::lookup(dict, molIdsHeader));
