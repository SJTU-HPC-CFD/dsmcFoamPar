...
:
    propsDict_(dict)
{
    PtrList<entry> infoList(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "data")));
    ...
    forAll(infoList, i)
    {
        ...
        scalar rhoN = Foam::hyCompat::toScalar(dictI, "numberDensity");

        density_[i] = rhoN;

        List<scalar> strain = List<scalar>(Foam::hyCompat::lookup(dictI, "strainRate"));
        List<scalar> viscosity = List<scalar>(Foam::hyCompat::lookup(dictI, "viscosity"));
        List<scalar> slipLength = List<scalar>(Foam::hyCompat::lookup(dictI, "slipLength"));
