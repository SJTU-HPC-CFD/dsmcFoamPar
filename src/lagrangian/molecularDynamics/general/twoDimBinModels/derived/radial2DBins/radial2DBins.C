...
defineTypeNameAndDebug(radial2DBins, 0);

addToRunTimeSelectionTable(twoDimBinModel, radial2DBins, dictionary);



// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

label radial2DBins::findBinR(const scalar& r)
{
    label n = -1;
...
label radial2DBins::findBinA(const scalar& theta)
{
...
:
    ...
    startPoint_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "startPoint"))),
    endPoint_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "endPoint"))),
    h_(mag(endPoint_ - startPoint_)),
    unitVector_((endPoint_ - startPoint_)/mag(endPoint_ - startPoint_)),

    angleUnitVectorY_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "angleUnitVectorY"))),
    angleUnitVectorX_(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "angleUnitVectorX"))),
    normalVector_(vector::zero),

    radius_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "radius"))),

//     Foam::hyCompat::lookup(nBinsL_(Foam::hyCompat::toLabel(Foam::hyCompat::lookup(propsDict_, "nBinsLength")))),
    nBinsR_(Foam::hyCompat::toLabel(Foam::hyCompat::lookup(propsDict_, "nBinsRadial"))),
    nBinsA_(Foam::hyCompat::toLabel(Foam::hyCompat::lookup(propsDict_, "nBinsAngle"))),
    ...
    n_()
{
    ...
    avVolume_ = radius_*radius_*constant::mathematical::pi*h_/(nBinsR_*nBinsA_);
...
//     {
//         normalVector_ = Foam::hyCompat::lookup(propsDict_, "normalVector");
//
...
List<label> radial2DBins::isPointWithinBin
(
    const vector& rI,
    const label& cellI
)
...
scalarField radial2DBins::binPositionsX()
{
    return magRadii_;
}

scalarField radial2DBins::binPositionsY()
{
...
const vector& radial2DBins::refVector() const
{
    return unitVector_;
}

List<label> radial2DBins::nBins()
{
    List<label> nBins;
nBins.append(-1);
nBins.append(-1);
...
scalar radial2DBins::binVolume(const label& n)
{
    return avVolume_;
}
