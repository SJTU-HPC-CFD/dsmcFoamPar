...
void timeDataMeas::setInitialData()
{
    ...
    if (timeDict_.found("resetAtOutput"))
    {
        ...
        if (timeDict_.readIfPresent("resetAtOutputUntilTime", resetAtOutputUntilTime_))
        {
            Info << "Resetting fields at output until time: "
                 << resetAtOutputUntilTime_ << endl;
        }
    }
    ...
    else if(timeMeasOption_ == "decoupledWrite")
    {
        ...
        const scalar couplingTimeInterval = timeCouplingDict.get<scalar>("couplingTimeInterval");
        const scalar molecularOnInterval = timeCouplingDict.get<scalar>("molecularOnInterval");
...
//         const scalar averagingTime = Foam::hyCompat::toScalar(Foam::hyCompat::lookup(timeCouplingDict, "couplingTimeInterval"));
        const scalar averagingTime = timeCouplingDict.get<scalar>("molecularCouplingTimeInterval");
...
//         checkAndModifyTimeProperties();
    ...
    else if(timeMeasOption_ == "decoupled")
    {
        ...
        const scalar averagingTime = timeCouplingDict.get<scalar>("molecularOnInterval");
    ...
    {
        const label noWriteIntervals = timeDict_.get<label>("noOfWriteIntervals");
        const label nSamplesDict = timeDict_.get<label>("nSamples");
...
const scalar& timeDataMeas::resetAtOutputUntilTime() const
{
    return resetAtOutputUntilTime_;
}
