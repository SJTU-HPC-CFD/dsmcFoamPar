    ...
    writeReferredCells_(false)
{
    ...
    {
        optimised_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propDict, "optimisedMesh")));
    }

    if (propDict.found("writeReferredCells"))
    {
        writeReferredCells_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propDict, "writeReferredCells")));
    }

    if (propDict.found("writeReferredCloud"))
    {
        write_ = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propDict, "writeReferredCloud")));
    }
...
//         << endl;
    ...
    forAll(points, p)
    {
        ...
        forAll(cells, c)
        {
            ...
            if(cellN != cellI)
            {
                if(Foam::hyCompat::indexOf(neighbCells, cellN) == -1)
                {
...
)
{
    ...
    if(Foam::hyCompat::indexOf(cells[cellN], cellI) != -1)
    {
        include = false;
    }

    // test also if cellI already has cellN
    if(Foam::hyCompat::indexOf(cells[cellI], cellN) != -1)
    {
        include = false;
    }
...
//     Info << "no of refCells after first cyclic boundary shift = " << nRefCells << endl;
    ...
    forAll(sourceCells, i)
    {
        forAll(sourceCells[i], j)
        {
            ...
            if(Foam::hyCompat::indexOf(ovSrcCells[cellI], i) == -1 )
            {
            ...
            forAll(sourceCells, k)
            {
                if(k != i)
                {
                    if(Foam::hyCompat::indexOf(sourceCells[k], cellI) != -1)
                    {
                        if(Foam::hyCompat::indexOf(ovSrcCells[cellI], k) == -1)
                        {
...
//                 Info << "pIN = " << pIN << endl;
                ...
                if(
                    ( Foam::hyCompat::indexOf(mod, pI) == -1 ) &&
                    ( Foam::hyCompat::indexOf(mod, pIN) == -1 )
                )
                {
                    mod.append(pI);
                    mod.append(pIN);
    ...
    if(Pstream::parRun())
    {
        ...
        forAll(mesh_.boundaryMesh(), patchI)
        {
            ...
            )
            {
                ...
                forAll(sProcCells, i)
                {
                    if(Foam::hyCompat::indexOf(srcCells, sProcCells[i]) == -1)
                    {
...
//         bb.expand(rCut_-offset);
        ...
        forAll(mesh_.cells(), c)
        {
            ...
            {
                if(Foam::hyCompat::indexOf(cells, c) == -1)
                {
                    cells.append(c);
                }
            }
