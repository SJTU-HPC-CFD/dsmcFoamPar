...
//     Foam::hyCompat::lookup(period_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "period")))),
    amplitude_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "amplitude"))),
//     offsetTime_(0.0),
    currentTime_(0.0),
//     currentTime_(time_.startTime().value()),
    deltaT_(Foam::hyCompat::toScalar(Foam::hyCompat::lookup(propsDict_, "deltaT")))
{
    unitVector_ /= mag(unitVector_);

    periodCoeffs_ = List<scalar>(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "coeffs")));
...
//     force_ = unitVector_*initialForce;
    ...
    {
        outputForces = Switch(Foam::hyCompat::lookup(Foam::hyCompat::lookup(propsDict_, "output")));
