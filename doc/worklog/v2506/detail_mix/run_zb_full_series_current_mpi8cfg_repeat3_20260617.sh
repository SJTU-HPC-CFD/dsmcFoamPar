...
OUTDIR="${OUTDIR:-$DETAIL/zb_full_series_current_mpi8cfg_repeat3_20260617}"
LOGDIR="$OUTDIR/logs"
...
backup_controls()
{
    ...
    do
        ...
        cp "$file" "$CTRLDIR/${label}_controlDict_before_20260617"
    done
}
...
restore_controls()
{
    ...
    do
        IFS='|' read -r label file <<< "$entry"
        backup="$CTRLDIR/${label}_controlDict_before_20260617"
        if [ -f "$backup" ]
        then
            cp "$backup" "$file"
        fi
...
prepare_origin_case()
{
    ...
    then
        (
            cd "$MPI8ORIGIN_CASE"
            decomposePar
        ) > "$LOGDIR/zb_MPI8origin_decomposePar_20260617.log" 2>&1
    fi
}
...
run_one()
{
    ...
    local ctrl_copy="$CTRLDIR/zb_${mode}_rep${rep}_controlDict"
    local log="$LOGDIR/zb_${mode}_rep${rep}.log"
    local exit_file="$LOGDIR/zb_${mode}_rep${rep}.exit"
...
do
    ...
    run_one "MPI2xOMP4" "$rep" "$MPI2OMP4_CASE" 2 4 1200
    run_one "MPI4xOMP2" "$rep" "$MPI4OMP2_CASE" 4 2 1200
    run_one "MPI8" "$rep" "$MPI8_CASE" 8 1 1200
